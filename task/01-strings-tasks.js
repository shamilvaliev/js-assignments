...
function extractNameFromTemplate(value) {
    let val = value.substr("Hello, ".length);
...
function encodeToRot13(str) {
    let res = '';
    let getCharByRanges = (ch, min, max) => {
        let intCh = ch.charCodeAt(0);
        if (intCh >= min && intCh <= max) {
            if (intCh + 13 > max) {
                intCh = min + intCh + 12 - max;
            } else {
                intCh += 13;
            }

            return String.fromCharCode(intCh);
        }
    ...
    let getChar = ch => {
        let val = getCharByRanges(ch, 65, 90);
        if (val == ch) {
            val = getCharByRanges(ch, 97, 122);
        }
